#include <ESP8266WiFi.h>
#include <WiFiClient.h>
#include <WiFiManager.h>
#include <PubSubClient.h>
#include <Ticker.h>
#include <dht11.h>
#include <SoftwareSerial.h>

#define dht11Pin 0

// MQTT Broker
const char *mqtt_broker = "k4c5aa11.cn-hangzhou.emqx.cloud";
const char *topic_pub = "env-perception/pub";
const char *topic_sub = "env-perception/sub";
const char *mqtt_username = "telecom";
const char *mqtt_password = "telecom";
const int mqtt_port = 15487;
const int device_id = 7003

WiFiClient espClient;
PubSubClient client(espClient);
SoftwareSerial mySerial(5,4); // D1:RX , D2:TX
dht11 dht;

Ticker ticker; // Ticker object for time-round function
String msg_pub = "7003,000,000,000";

void setup() {
  //初始化串口设置
  Serial.begin(9600);

  // Initialize the LED_BUILTIN pin as an output
 
  // Instantiate a WiFiManager object
  WiFiManager wifiManager;

  //  wifiManager.resetSettings();
  
  // Auto-connecting WiFi
  // The argument is AP's name when we connecting to ESP8266
  wifiManager.autoConnect("Group23-AutoConnectAP");

  // WiFi
  Serial.print("WiFi Connected!\n");
  String ssid = wifiManager.getWiFiSSID(); // Enter your WiFi name
  String password = wifiManager.getWiFiPass(); // Enter WiFi password

  // connecting to a WiFi network
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
      delay(500);
      Serial.println("Connecting to WiFi..");
  }
  Serial.println("Connected to the WiFi network");
  //connecting to a mqtt broker
  client.setServer(mqtt_broker, mqtt_port);
  client.setCallback(callback);
  while (!client.connected()) {
      String client_id = "esp8266-client-";
      client_id += String(WiFi.macAddress());
      Serial.printf("The client %s connects to the public mqtt broker\n", client_id.c_str());
      if (client.connect(client_id.c_str(), mqtt_username, mqtt_password)) {
          Serial.println("Emqx mqtt broker connected");
      } else {
          Serial.print("failed with state ");
          Serial.print(client.state());
          delay(2000);
      }
  }
  // publish and subscribe
  client.publish("hello", "hello emqx");
  client.subscribe(topic_sub);

  // Run data_publishment once in each seconds
  ticker.attach(1, data_publishment);

  mySerial.begin(9600);
}

// the loop function runs over and over again forever
void loop() {
  client.loop();

  // gather data
  // data_collection();

  // send to LCD slave
  data_pass2slave();
}

int lumi; // light luminance
int humi; // huminity
int temp; // temperature
void data_collection(){
  msg_pub = "";
  // Read luminosity, moisture and temperature
  lumi = analogRead(2) / 4;
  dht.read(dht11Pin);    //将读取到的值赋给tol
  
  temp = (float) dht.temperature; //将温度值赋值给temp
  humi = (float) dht.humidity; //将湿度值赋给humi

  msg_pub += device_id + ",";
  msg_pub += temp + ",";
  msg_pub +=  humi + ",";
  msg_pub += lumi;

  Serial.println(msg_pub);
  delay(10);      //延时1秒
}

void data_pass2slave(){
  String msg_to_lcd = "000000000110";
  // do sth
  mySerial.println(msg_to_lcd);
  delay(10);
}

void data_publishment(){
  Serial.println("Msg published successfully!");
  client.publish(topic_pub, msg_pub.c_str());
}

void callback(char *topic, byte *payload, unsigned int length) {
  Serial.print("Message arrived in topic: ");
  Serial.println(topic);
  Serial.print("Message:");
  for (int i = 0; i < length; i++) {
     Serial.print((char) payload[i]);
  }
  Serial.println();
  Serial.println("-----------------------");
}
